...
%
% Purpose: 
% Compute the maximum series gain (alpha) when using the Small Gain Theorem
% as defined by Equation 5.15 in Boyd, 1994.
%
% Parameters:
...
function [alpha_low,data,dec]=SGT(syst,eps,alpha_up)
...
c1 = P >= epsilon*In; % PD
c2 = V >= epsilon*Im; % PD
c3 = [A'*P + P*A + C'*V*C, P*B;
      B'*P, -V] <= -epsilon*eye(n+m); % ND
